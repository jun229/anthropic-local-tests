...
def parse_job_sections(html_content):
    ...
    elements = soup.find_all(['p', 'h1', 'h2', 'h3', 'h4', 'ul', 'ol'])
    
    current_section = "intro"
    intro_paragraphs = []
    
    for element in elements:
        # Handle headers - these indicate section changes
        if element.name in ['h1', 'h2', 'h3', 'h4']:
            ...
            # Determine section based on header
            if any(word in header_text for word in ['responsibilities', 'responsibi', 'duties', 'what you', 'you will', 'role']):
                current_section = "responsibilities"
            elif any(word in header_text for word in ['requirements', 'qualifications', 'must have', 'required', 'minimum', 'essential', 'experience', 'skills']):
                current_section = "requirements"
            elif any(word in header_text for word in ['nice to have', 'bonus', 'preferred', 'plus', 'nice-to-have', 'would be nice', 'great to have']):
                current_section = "nice_to_haves"
            # If header doesn't match known patterns, try to infer from content
            elif any(word in header_text for word in ['about', 'who we', 'we are', 'company', 'team']):
                current_section = "intro"
...
def create_job_file_content(job_data):
    ...
    if sections['nice_to_haves']:
        for item in sections['nice_to_haves']:
            content.append(f"- {item}")
    else:
